defmodule Ryal.PaymentGateways.Stripe do
  @moduledoc "Relevant functions for working with the Stripe API."

  alias Ryal.Config

  @behaviour Ryal.PaymentGateways

  @stripe_api_key Config.payment_gateway(:stripe)[:token]
  ...
  def update(:customer, %{external_id: external_id, user: user}, stripe_base) do
    response =
      HTTPotion.post("#{stripe_base}/v1/customers/#{external_id}", body: params(:customer, user))
  ...
  def delete(:customer, %{external_id: external_id}, stripe_base) do
    response = HTTPotion.delete("#{stripe_base}/v1/customers/#{external_id}")
    Jason.decode(response.body)
  end
